...
function displayAllResults(e){
    ...
    const searchResults = obtainSearchResults() 
...
function obtainSearchResults(){
    ...
    if stringEntered.includes(":"){
        const wildcard = getWildcard(stringEntered)
        const query = getQuery(stringEntered) 
    } 
    
    const url = '/users/:' + wildcard + "/:" + query;
...
function getQuery(stringEntered){
    var indexOfColon = stringEntered.indexOf(":"); 
    var query = stringEntered.slice(indexOfColon, stringEntered.length); 
}
